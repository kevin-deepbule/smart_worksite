...
- 合规审查：只加载已启用审查模板，提交审查、查询审查结果；页面提供上传模板按钮，跳转模板中心并预筛选审查模板、打开上传弹窗；审核历史组件按当前项目分页查询，可按状态筛选并选择历史记录查看审核结论、规则结果、问题列表、JSON 结果和任务阶段；切换项目时清空上一项目详情；已完成审查的问题状态可按后端枚举更新为 `OPEN`、`PROCESSING`、`RESOLVED` 或 `IGNORED`。
