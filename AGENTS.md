...
- Review submission is asynchronous: reuse the file module to parse DOC, DOCX, PDF, UTF-8 TXT, XLS, and XLSX into Markdown, keep the `REVIEW_EXECUTION` task `PENDING` while parsing, and write the task outbox only after a complete non-truncated Markdown result is ready.
- Long-document review persists `review_record`, `review_document_chunk`, and `review_rule_result`. Do not create a full rule-by-chunk result table; keep block observations transient and persist only effective evidence in `review_rule_result.evidence_json` with same-review `chunkId` references.
- Review workers must evaluate chunk evidence before reducing each rule to `COMPLIANT`, `NON_COMPLIANT`, or `PARTIALLY_COMPLIANT`; non-compliant and partially compliant results require a reason and suggestion. Java deterministically computes the overall status and the final model call may only produce the summary.
...
- Report template uploads must scan real `{{ var_xx_xx }}` placeholders before object storage upload and persist the ordered unique variable names into `template_variable_description` with an empty initial description after template/file IDs exist. Review template uploads must scan real `{{ var_xx_xx : review rule description }}` definitions before object storage upload, require at least one non-blank rule, and persist the ordered unique variable names with their non-blank descriptions into the same table. Review duplicate variables are allowed only when their normalized descriptions are identical. Parse or persistence failures must fail the upload visibly, roll back database writes, and clean up the uploaded object where applicable.
...
- Long-document review workers must renew the owned task lease before each chunk model call and check cooperative cancellation at the same boundary.
...
- The compliance review page must provide a project-scoped, paginated review-history component. Selecting a history record must load its persisted detail, rule results, issues, and task stages through Java backend APIs; project switches must clear stale detail from the previous project.
...
- Template center must display the backend `templateId` and fetch preview content only through the Java `GET /api/templates/{templateId}/preview` API. DOCX, XLSX/CSV, and text previews should be rendered locally from the returned Blob, and unsupported formats or preview failures must remain visible to users. REPORT rows must expose a template-variable dialog that loads current variable descriptions from Java, keeps variable names read-only, and submits the complete non-blank description list through the backend update API. REVIEW rows use the same interaction for parsed review rules: variable names remain read-only, rule descriptions are editable, and the complete non-blank rule list is submitted through the same backend update API.
