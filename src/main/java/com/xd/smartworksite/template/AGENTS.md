...
## Template Upload Auto Parsing

- 报告模板和审查模板上传都必须在对象存储写入前自动解析；重新解析接口和解析状态字段暂不实现。
...
- 所有进入统一上传应用服务且 `templateCategory = REVIEW` 的入口都必须解析 `{{ var_xx_xx : 审核规则描述 }}`，包括 `/api/templates`、`/api/templates/review` 和 `/api/review/templates`。
- 审查模板至少包含一个审核规则；变量名和描述均不能为空，描述最长 2000 字符。第一个英文冒号分隔变量名与描述，描述内可继续使用中英文冒号。
- 审查模板同名变量重复出现时，描述完全一致才允许按首次出现顺序去重；同名不同描述必须拒绝上传。
- 模板文件、`file_object`、`template` 和解析变量必须全部成功后才能返回上传成功。数据库变量写入失败时数据库事务必须回滚，并尽力清理本次已上传的 MinIO 对象。
- 自动解析得到的变量按扫描器返回顺序写入现有 `template_variable_description` 表；本阶段不新增变量表。报告模板首次写入时 `description` 使用空字符串表示“变量已解析但尚未配置描述”。
- 审查模板的审核规则描述直接写入 `template_variable_description.description`，不得写入空描述；规则必须在执行合规审查时通过模板应用服务传递给 Python Agent。
...
- 报告模板变量只识别 `{{ var_xx_xx }}`；审查模板变量只识别 `{{ var_xx_xx : 审核规则描述 }}`。
...
- 描述查询必须按模板类别重新扫描当前模板文件并按 `template_id + file_id + variable_name` 合并已持久化描述；审查模板缺少持久化记录时回显文件中解析出的规则描述。
- 变量描述写入接口为 `PUT /api/templates/{templateId}/variables/descriptions`。
- 变量描述写入接口同时允许修改报告模板描述和审查模板规则描述；两类模板都必须重新扫描文件，并要求请求覆盖文件中的全部唯一变量，变量名只读且不能增删。
...
- 前端模板中心对 `REPORT` 模板展示可编辑的“模板变量”入口，对 `REVIEW` 模板展示同样可编辑的“审核规则”入口；两者变量名只读、描述必填并整组保存。
...
- 测试必须覆盖正常顺序、重复变量、跨 Run 变量、审查规则内冒号、审查同名不同描述、审查无规则、非法变量、缺失描述、未知描述变量、重复提交更新、跨项目访问、PDF 拒绝和 MinIO 失败。
