...
- 合规审查支持 DOC、DOCX、PDF、UTF-8 TXT、XLS、XLSX，复用文件解析结果并通过 `REVIEW_EXECUTION` 任务异步执行 Markdown 分块、块级证据识别、规则三态汇总和最终摘要。
- 模板上传、列表、详情、修改、启用、停用、删除和项目级访问校验。
- 报告模板上传前自动扫描并持久化 `{{ var_xx_xx }}` 变量；审查模板上传前自动扫描 `{{ var_xx_xx : 审核规则描述 }}` 并持久化非空规则描述。模板中心支持模板文件流预览、变量顺序查询，以及报告变量描述和审查规则维护。
- 报告模板和审查模板兼容接口。
...
| GET | `/api/templates/{templateId}/variables` | 按模板类别扫描报告变量或审查规则变量并按首次出现顺序去重 |
...
模板变量解析规则：变量接口必须读取模板文件真实内容；报告模板只识别 `{{ var_xx_xx }}`，审查模板只识别 `{{ var_xx_xx : 审核规则描述 }}`。当前支持 DOC、DOCX、XLS、XLSX、CSV、TXT、MD，不支持 PDF；模板文件缺失、跨项目不一致、格式损坏、格式不支持或对象存储读取失败时直接返回错误，不允许用空列表隐藏解析失败。

模板写入规则：报告模板上传在写入 MinIO 前自动扫描真实文件变量，并在文件、模板记录生成 ID 后将变量以空描述写入 `template_variable_description`。审查模板上传必须在写入 MinIO 前解析至少一个 `{{ var_xx_xx : 审核规则描述 }}`，将非空规则描述写入同一张表；同名变量只有在描述一致时才允许去重。解析或变量持久化失败时上传失败，数据库写入回滚并清理本次 MinIO 对象。模板上传创建后必须读回持久化记录再返回成功；模板修改、启用、停用和删除必须检查数据库影响行数，记录不存在或状态已变化时直接返回冲突错误，不允许静默成功。
...
FILE_PARSE_MAX_INPUT_CHARS=120000
FILE_PARSE_MAX_DOCUMENT_CHARS=2000000
REVIEW_PARSE_POLL_LIMIT=20
REVIEW_PARSE_POLL_DELAY_MS=2000
REVIEW_MAX_MODEL_CALLS=10000
REVIEW_CHUNK_MAX_CHARS=6000
REVIEW_CHUNK_OVERLAP_CHARS=600
REVIEW_MAX_CHUNKS=500
REVIEW_MAX_DOCUMENT_CHARS=2000000
```
...
Review read APIs require `review:view`; submit/retry/cancel/delete/archive/update-issue APIs require `review:manage`.

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/review/records` | Submit a DOC/DOCX/PDF/TXT/XLS/XLSX file and create an async review |
| GET | `/api/review/records` | List review records |
| GET | `/api/review/records/{recordId}` | Get review record detail and issues |
| GET | `/api/review/records/{recordId}/rule-results` | List rule snapshots, final statuses, reasons and evidence |
| GET | `/api/review/records/{recordId}/rule-results/{ruleCode}` | Get one rule result |
| GET | `/api/review/records/{recordId}/stages` | List review task stage logs |
| POST | `/api/review/records/{recordId}/retry` | Retry failed review |
| POST | `/api/review/records/{recordId}/cancel` | Cancel a pending or running review |
...
审查持久化采用 `review_record`、`review_document_chunk`、`review_rule_result` 三表；块级无证据观察不落库，只有有效证据通过 `evidence_json.chunkId` 引用同一审核下的文档块。
...
| `docs/智慧工地大模型应用系统-系统设计文档.md` | 基于当前代码与目标边界的完整系统设计，包含架构、流程、数据、安全、部署和 Mermaid 图 |
